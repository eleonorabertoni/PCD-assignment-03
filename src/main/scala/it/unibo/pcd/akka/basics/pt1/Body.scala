...
object Body :
  ...
  case class BodyImpl(id: Int, pos: P2d, vel: V2d, mass: Double) extends Body:
     ...
     */
    def updatePos(dt: Double) = pos.sum(V2d(vel).scalarMul(dt))
    /**
     * Update the velocity, given the instant acceleration
     * @param acc instant acceleration
     * @param dt time elapsed
     */
    def updateVelocity(acc: V2d, dt: Double) = vel.sum(V2d(acc).scalarMul(dt))
    /**
     * Change the velocity
     *
     * @param vx
     * @param vy
     */
    def changeVel(vx: Double, vy: Double) = vel.change(vx, vy)
    /**
